...
export function EventTeamContent({
  ...
  const [blockedSlots, setBlockedSlots] = useState<
    Array<{ date: string; hour: string; stageId: string }>
  >([]);
  ...
  const handleDrop = (day: string, time: string, stageId: string) => {
    ...
    if (draggedSlot) {
      const durationMinutes =
        timeToMinutes(draggedSlot.endTime) -
        timeToMinutes(draggedSlot.startTime);
  ...
  const handleSaveEdit = () => {
    ...
    const hasCollision = schedule.some((slot) => {
      if (
        ...
        slot.stageId !== editingSlot.stageId
      )
        return false;
      const slotStartMinutes = timeToMinutes(slot.startTime);
      const slotEndMinutes = timeToMinutes(slot.endTime);
  ...
  const handleToggleBlockSlot = (date: string, hour: string) => {
    ...
    if (isBlocked) {
      ...
      setBlockedSlots([
        ...blockedSlots,
        { date, hour, stageId: selectedStage },
      ]);
    }
  };
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {producers.length} productor{producers.length !== 1 ? "es" : ""}{" "}
                asignado{producers.length !== 1 ? "s" : ""}
              </p>
            </div>
            <AddProducerDialog
              eventId={eventId}
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {producers.map((item) => {
                ...
                return (
                  <Card
                    key={item.id}
                    className="group relative overflow-hidden rounded-2xl bg-white/[0.02] backdrop-blur-xl border-white/5 hover:border-white/10 transition-all duration-300"
                  >
        ...
        <div>
          <div className="mb-4">
            <h3 className="text-lg font-semibold">
              Artistas, Performers, Speakers, DJs, Conferencistas
            </h3>
            <p className="text-sm text-muted-foreground">
              {artists.length} artista{artists.length !== 1 ? "s" : ""} asignado
              {artists.length !== 1 ? "s" : ""}
            </p>
          </div>
              ...
              >
                <Card className="bg-white/[0.02] border-white/5 overflow-hidden flex-1">
                  <div className="overflow-x-auto overflow-y-visible">
                    <div
                      style={{ minWidth: `${(eventDays.length + 1) * 150}px` }}
                    >
                      {/* Header con días */}
                      <div
                      ...
                      >
                        ...
                        {eventDays.map((day) => (
                          <div
                            key={day.date}
                            className="p-3 bg-white/[0.02] border-r border-white/5 last:border-r-0"
                          >
                          ...
                          return (
                            <Fragment key={`hour-${hour}`}>
                              ...
                              <div
                                ...
                                className={`p-3 bg-white/[0.02] border-r border-white/5 border-b  flex items-center group transition-colors ${
                                  hasSlots
                                    ? "cursor-pointer hover:bg-red-500/10 hover:border-red-500/20"
                                    : ""
                                }`}
                                style={{
                                  gridColumn: 1,
                                ...
                                title={
                                  hasSlots
                                    ? "Click para eliminar todos los artistas de esta hora"
                                    : ""
                                }
                              >
                              ...
                              {eventDays.map((day, dayIndex) => {
                                ...
                                const slotsInThisHour = getSlotsInHour(
                                  day.date,
                                  hour,
                                  selectedStage
                                );
                                ...
                                if (!available) {
                                  return (
                                    <div
                                      key={`${day.date}-${hour}`}
                                      className="p-2 border-r border-white/5 last:border-r-0 border-b  bg-white/[0.01]"
                                      style={{
                                ...
                                const previewHour = dragPreview?.time
                                  ? dragPreview.time.split(":")[0] + ":00"
                                  : null;
                                const isPreview =
                                  dragPreview?.day === day.date &&
                                  previewHour === hour &&
                                  dragPreview?.stage === selectedStage;
                                ...
                                return (
                                  <div
                                    ...
                                    onDragOver={
                                      blocked ? undefined : handleDragOver
                                    }
                                    onDrop={
                                      blocked
                                        ? undefined
                                    ...
                                    className={`border-r border-white/5 last:border-r-0 border-b  relative overflow-visible transition-all ${
                                  ...
                                  >
                                    {slotsInThisHour.length > 0 ? (
                                      slotsInThisHour.map((slot) => {
                                        const topPosition = getPositionInHour(
                                          slot.startTime
                                        );
                                      ...
                                      <div
                                        ...
                                        title={
                                          blocked
                                            ? "Click para desbloquear este horario"
                                            : ""
                                        }
                                      >
                                        {blocked ? (
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg bg-white/[0.02] border border-white/5">
              ...
              <div>
                ...
                <p className="text-sm text-white/60">
                  {eventDays.find((d) => d.date === editingSlot?.day)?.dayName}{" "}
                  {
                    eventDays.find((d) => d.date === editingSlot?.day)
                      ?.shortDate
                  }
                </p>
                  ...
                  <SelectContent>
                    {eventDays
                      ...
                      ?.timeSlots.map((time) => {
                        ...
                        const endTime = `${String(endHour).padStart(2, "0")}:${String(endMinute).padStart(2, "0")}`;
                        return (
                          <SelectItem key={`end-${time}`} value={endTime}>
