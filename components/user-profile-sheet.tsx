...
import { Mail, Phone, FileText, Calendar, Cake, ShoppingBag, Ticket, Bell, BellOff } from "lucide-react";
...
export function UserProfileSheet({ user, open: controlledOpen, onOpenChange }: UserProfileSheetProps) {
  ...
  const loadTransactions = async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, ANIMATION_DELAYS.MOCK_LOADING));

    // TODO: Replace with real data fetch
    // For now using mock data from centralized location
    const { MOCK_TRANSACTIONS } = await import("@/lib/users/mock-data");

    const mockTotalSpent = MOCK_TRANSACTIONS.reduce((sum, tx) => sum + tx.total, 0);
    const mockTotalTickets = MOCK_TRANSACTIONS.reduce((sum, tx) => sum + tx.quantity, 0);
      ...
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto bg-white dark:bg-[#1a1a1a] border-gray-200 dark:border-[#2a2a2a] p-0 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <div className="p-6 space-y-6">
          <SheetHeader className="space-y-2 p-0">
            <SheetTitle className="text-2xl font-bold text-gray-900 dark:text-white">Perfil de Usuario</SheetTitle>
          ...
          <div className="flex items-start gap-4 p-6 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-3">{fullName}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {user.document_id && (
              <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5">
                ...
                <div className="text-sm font-semibold text-gray-900 dark:text-white">{user.document_id}</div>
              </div>
            )}
            ...
            <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5">
              ...
              <div className="flex items-center gap-2">
                <div className={`text-sm font-semibold ${user.marketing_emails ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Estadísticas de Compra</h3>
          ...
          {/* Transaction History */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Historial de Compras</h3>

            {loading ? (
              <div className="text-center py-12 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5" role="status" aria-live="polite">
                <div className="h-8 w-8 border-2 border-gray-300 dark:border-white/30 border-t-gray-900 dark:border-t-white rounded-full animate-spin mx-auto" aria-hidden="true" />
                <p className="text-sm text-gray-500 dark:text-white/40 mt-3">Cargando transacciones...</p>
              </div>
            ) : error ? (
              <div className="text-center py-12 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20" role="alert">
                ...
                {transactions.map((tx) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-gray-900 dark:text-white truncate mb-1">{tx.event_name}</div>
                        <div className="text-sm text-gray-600 dark:text-white/60 truncate">{tx.ticket_name}</div>
                      </div>
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-bold text-gray-900 dark:text-white">${tx.total.toLocaleString('es-CO')}</span>
                    </div>
              ...
              <div className="text-center py-16 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5">
                ...
                <p className="text-sm text-gray-500 dark:text-white/40">Este usuario no ha realizado compras</p>
              </div>
            )}
          </div>
        </div>
