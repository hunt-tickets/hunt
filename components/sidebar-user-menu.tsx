...
export function SidebarUserMenu({ user, userId, onMenuClose }: SidebarUserMenuProps) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors group w-full text-left"
        >
          {user?.image ? (
            <img
              src={user.image}
      ...
      >
        <div className="px-3 py-3 border-b dark:border-zinc-800">
          ...
          <p className="text-xs text-gray-400 mt-0.5">{user?.email || "usuario@email.com"}</p>
        </div>

        <div className="p-1">
          <DropdownMenuItem asChild className="rounded-xl cursor-pointer">
            <Link href={`/profile/`} onClick={onMenuClose} className="flex items-center px-3 py-2">
              <User className="mr-2 h-4 w-4" strokeWidth={1.5} />
              <span>Mi Perfil</span>
            </Link>
          </DropdownMenuItem>
          <DropdownMenuItem asChild className="rounded-xl cursor-pointer">
            <Link href={`/profile/${userId}/organizaciones`} onClick={onMenuClose} className="flex items-center px-3 py-2">
          ...
          <DropdownMenuItem asChild className="rounded-xl cursor-pointer">
            <Link href={`/profile/${userId}/entradas`} onClick={onMenuClose} className="flex items-center px-3 py-2">
              <svg className="mr-2 h-4 w-4" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
