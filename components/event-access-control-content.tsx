...
export function EventAccessControlContent({
  qrCodes,
  transactionsWithoutQR,
  eventId,
  showTabsOnly = false,
  showContentOnly = false,
}: EventAccessControlContentProps) {
  const { accessControlTab: mainTab, setAccessControlTab: setMainTab } =
    useEventTabs();
  const [searchTerm, setSearchTerm] = useState("");
  const [activeTab, setActiveTab] = useState<
    "all" | "scanned" | "pending" | "noqr"
  >("all");
  ...
  const handleDownloadCSV = () => {
    const dataToExport =
      activeTab === "noqr" ? transactionsWithoutQR : filteredQRCodes;
    ...
    if (activeTab === "noqr") {
      ...
      csvContent =
        "Nombre,Email,Entrada,Comprados,Generados,Faltantes,ID Pedido\n";
      ...
      // CSV for QR codes
      csvContent =
        "Entrada,Usuario,Email,Estado,Escaneado por,Email escaneador,Pedido,Fuente,Fecha creación,Fecha actualización\n";
      filteredQRCodes.forEach((qr) => {
        const status = qr.scan ? "Escaneada" : "Pendiente";
        const scanDate = qr.updated_at
          ? new Date(qr.updated_at).toLocaleString("es-CO")
          : "";
  ...
  const filteredQRCodes = useMemo(() => {
    ...
    return tabFilteredQRCodes.filter(
      (qr) =>
  ...
  const stats = useMemo(() => {
    ...
    const scanRate =
      localQRCodes.length > 0 ? (scanned / localQRCodes.length) * 100 : 0;
