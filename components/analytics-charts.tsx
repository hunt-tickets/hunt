          ...
          {(ageGroups || []).length === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-xs text-gray-400 dark:text-white/30 mt-1">
                Los datos aparecerán cuando haya usuarios con compras
              </p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-white/5">
                    <th className="text-left py-3 px-4 text-sm font-medium text-gray-500 dark:text-white/60">
                      Grupo de Edad
                    </th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-gray-500 dark:text-white/60">
                      Usuarios
                    </th>
                    <th className="text-right py-3 px-4 text-sm font-medium text-gray-500 dark:text-white/60">
                      Tickets
                ...
                <tbody>
                  {ageGroups.map((group) => (
                    <tr
                      key={group.ageGroup}
                      className="border-b border-gray-200 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-[#202020]"
                    >
                ...
                <tfoot>
                  <tr className="border-t border-gray-300 dark:border-white/10 font-medium">
                    ...
                    <td className="py-3 px-4 text-sm text-right">
                      {(() => {
                        const totalPrice = ageGroups.reduce(
                          (sum, group) =>
                            sum + (group.averagePrice || 0) * group.users,
                          0
                        );
                        const avgPrice =
                          totalUsers > 0 ? totalPrice / totalUsers : 0;
                        return avgPrice > 0
                          ? new Intl.NumberFormat("es-CO", {
                              style: "currency",
                              currency: "COP",
